...
[ha, pos] = tight_subplot(1,2,[0 0],[.1 .05],[.08 .01])
...
text([0.68,0.86,1.05,1.25],[-170,-170,-170,-170],{'CTB','RTB','REW','NDC'})
text([0.68,0.86,1.05,1.25]+1,[-170,-170,-170,-170],{'CTB','RTB','REW','NDC'})
hold off;

axes(ha(2));
hold on;
b_CN = bar(TR_WF_EDC_CN,'EdgeColor','none','BarWidth', 1);
for k = 1:1:5
    set(b_CN(:,k),'facecolor',c(k,:))
end

errorbar(x_CN,y_CN,err_CN_pos,err_CN_neg,'o','Color','k','LineWidth',1);
errorbar(x_CNP,y_CNP,err_CN_posP,err_CN_negP,'o','Color','k','LineWidth',1);

ylim([-5000,3000]);
%ytick_CN = get(gca,'YLim');
set(gca,'YTick',ytick_both)
set(gca,'XTickLabel',xtick_both,'FontSize',12)

pos_CN = get(gca,'Position')
%ytick_CN = get(gca,'Ytick');
title('China','FontSize',16);
text([0.68,0.86,1.05,1.25],[-170,-170,-170,-170],{'CTB','RTB','REW','NDC'})
text([0.68,0.86,1.05,1.25]+1,[-170,-170,-170,-170],{'CTB','RTB','REW','NDC'})
hold off;
...
legend({'Crop Trade Balance (CTB)','Regional Trade Balance (RTB)',...
    ...
    'N Surplus Economic Damage Cost (NDC)'},'Location','southeast','FontSize',12)
