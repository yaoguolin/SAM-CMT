function [h]=RESULTS_decomp_graph07202020_errorbar(var_dep,var_tot,err_glb,tlt,y_label,country_label,crop_label)
dim = size(var_dep);
co_dim = dim(1);
cr_dim = dim(2);
c = ...
...
b_p = bar(1:co_dim+1,var_dep_pos,'stacked','BarWidth',0.7);
hold on;
b_n = bar(1:co_dim+1,var_dep_neg,'stacked','BarWidth',0.7);
...
set(gca, 'FontSize', 12)
hold on;
s = scatter(1:co_dim+1,var_tot_global,30,'k','filled')
error_bar = errorbar(1:co_dim+1, var_tot_global, err_glb, 'Color', 'k', 'LineStyle','none', 'CapSize',25);
h = [b_p,s, error_bar];
label = horzcat(crop_label, "Total");
%leg1 = legend(h,label,'Location','northeastoutside');
ylabel(y_label);
title(tlt,'FontSize',13);
hold off;
