...
WF_Blue_m3ton_56(1:218,1:170,1:54) = WF_Blue_m3ton;
...
BlueW_Agg = Var_Agg_WGT(WF_Blue_m3ton_56,Product_FAO,MAPPING);
BlueW_Agg(BlueW_Agg==0) = nan;
BlueW_Agg = BlueW_Agg(:,cr_ind,:);

BlueW_L_Agg = BlueW_Agg.*Product_FAO_Agg;
BlueW_L_AggCO =squeeze(nansum(BlueW_L_Agg,2));

%%% Blue water intensity by country
BlueW_AggCO = BlueW_L_AggCO ./ Product_FAO_AggCO;
...
% New
BlueW_L_Exp = BlueW_Agg(:,:,yr_base).*prod_port;
...
BlueW_ExpCO = BlueW_L_ExpCO ./ prod_portco;

%%% Blue water intensity globally after shocks
BlueW_ExpGB = BlueW_L_ExpGB ./ prod_portgb;

% Changes
BlueW_L_Delta = BlueW_Agg(:,:,yr_base).* prod_delta;
...
for az = 1:1:18
    BlueW_L_AEZ_Agg(:,az,:,:) = squeeze(BlueW_Agg(:,:,:)) .* squeeze(Prod_AEZ_FAO_Agg(:,az,:,:));
end

% New
BlueW_L_AEZ_Exp(1:co_dim,1:18,cr_dim) = nan;
for az = 1:1:18
    BlueW_L_AEZ_Exp(:,az,:) = squeeze(BlueW_Agg(:,:,yr_base)) .* squeeze(prod_aez_port(:,az,:));
end
