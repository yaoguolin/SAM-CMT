...
h2oDeplete_3d_gf_Agg = Var_Agg_WGT(h2oDeplete_Lkm,AreaH_FAO,MAPPING);
% h2oDeplete_3d_gf_Agg(h2oDeplete_3d_gf_Agg==0) = nan;
h2oDeplete_3d_gf_Agg = h2oDeplete_3d_gf_Agg(:,cr_ind,:);
h2oDeplete_3d_L_Agg = h2oDeplete_3d_gf_Agg.*AreaH_FAO_Agg;
h2oDeplete_L_AggCO =squeeze(nansum(h2oDeplete_3d_L_Agg,2));

%%% Ground water intensity by country
h2oDeplete_AggCO = h2oDeplete_L_AggCO ./ AreaH_FAO_AggCO;

%%%% Global
h2oDeplete_L_AggGB = nansum(h2oDeplete_AggCO,1);

%%% Ground water intensity globally before shocks
h2oDeplete_AggGB = h2oDeplete_L_AggGB ./ AreaH_FAO_AggGB;

% New
h2oDeplete_3d_L_Exp = h2oDeplete_3d_gf_Agg(:,:,yr_base).*harvst_port;
...
h2oDeplete_ExpCO = h2oDeplete_L_ExpCO ./ harvst_portco;

%%% Blue water intensity globally after shocks
h2oDeplete_ExpGB = h2oDeplete_L_ExpGB ./ harvst_portgb;

% Changes
h2oDeplete_L_Delta = h2oDeplete_3d_gf_Agg(:,:,yr_base).* harvst_delta;
...
for az = 1:1:18
    h2oDeplete_L_AEZ_Agg(:,az,:,:) = squeeze(h2oDeplete_3d_gf_Agg(:,:,:)) .* squeeze(AreaH_AEZ_FAO_Agg(:,az,:,:));
end

h2oDeplete_Lkm_AEZ_Agg = h2oDeplete_L_AEZ_Agg ./ AreaH_AEZ_FAO_Agg;

% New
h2oDeplete_L_AEZ_Exp(1:co_dim,1:18,cr_dim) = nan;
for az = 1:1:18
    h2oDeplete_L_AEZ_Exp(:,az,:) = squeeze(h2oDeplete_3d_gf_Agg(:,:,yr_base)) .* squeeze(harvst_aez_port(:,az,:));
end

h2oDeplete_Lkm_AEZ_Exp = h2oDeplete_L_AEZ_Exp ./ harvst_aez_port;


% Changes
h2oDeplete_L_AEZ_Delta = h2oDeplete_L_AEZ_Exp - h2oDeplete_L_AEZ_Agg(:,:,:,yr_base);

h2oDeplete_Lkm_AEZ_Delta = h2oDeplete_Lkm_AEZ_Exp - h2oDeplete_Lkm_AEZ_Agg(:,:,:,yr_base);
...
h2oDeplete_Lkm_AEZ_AggCO = h2oDeplete_L_AEZ_AggCO ./AreaH_AEZ_FAO_AggCO; 
h2oDeplete_Lkm_AEZ_ExpCO = h2oDeplete_L_AEZ_ExpCO ./harvst_aez_portco;
h2oDeplete_Lkm_AEZ_DeltaCO = h2oDeplete_Lkm_AEZ_ExpCO - h2oDeplete_Lkm_AEZ_AggCO(:,:,yr_base);
...
h2oDeplete_Lkm_AEZ_AggGB = h2oDeplete_L_AEZ_AggGB ./AreaH_AEZ_FAO_AggGB;
h2oDeplete_Lkm_AEZ_ExpGB = h2oDeplete_L_AEZ_ExpGB ./harvst_aez_portgb;
h2oDeplete_Lkm_AEZ_DeltaGB = h2oDeplete_Lkm_AEZ_ExpGB - h2oDeplete_Lkm_AEZ_AggGB(:,yr_base);
